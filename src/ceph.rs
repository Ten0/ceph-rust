...
use {
    crate::{admin_sockets::*, error::*, json::*, JsonValue},
    byteorder::{LittleEndian, WriteBytesExt},
    futures::task::SpawnExt,
    libc::*,
    nom::{number::complete::le_u32, IResult},
    serde_json,
};
...
use {
    crate::{completion::with_completion, rados::*, status::*},
    std::{
        ffi::{CStr, CString},
        marker::PhantomData,
        ptr, str,
...
impl IoCtx {
    ...
    ///
    /// Efficiency: If size_hint is not specified, and this function is used on a small object, it
    /// will issue spurious read-ahead operations beyond the object's size.
    /// If you have an object that you know is small, prefer to use a single
    /// `rados_async_object_read` instead of this streaming variant.
    ///
    /// * `buffer_size` - How much data should be read per rados read operation.  This is also how
    ///   much data is emitted in each Item from the stream.
    /// * `concurrency` - How many RADOS operations should be run in parallel for this stream, or
    ///   None to use a default.
    /// * `size_hint` - If you have prior knowledge of the object's size in bytes, pass it here to
    ///   enable the stream to issue fewer read-ahead operations than it would by default.  This is
    ///   just a hint, and does not bound the data returned -- if the object is smaller or larger
    ...
    /// Streaming write of a RADOS object.  The `WriteSink` object implements `futures::Sink`.
    /// Combine it with other stream-aware code, or bring the SinkExt trait into scope to get
    /// methods like send, send_all.
    ///
